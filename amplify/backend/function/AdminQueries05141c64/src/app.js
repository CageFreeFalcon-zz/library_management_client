...
app.post("/verifyUser", async (req, res, next) => {
  ...
  try {
    await addUserToGroup(req.body.username, "students");
    await removeUserFromGroup(req.body.username, "requests");
    const payload = {
    ...
    const { UserAttributes } = await getUser(req.body.username);
    UserAttributes.forEach(obj => {
      if (obj.Name === "name") {
        payload["name"] = obj.Value;
      }
      if (obj.Name === "custom:department") {
        payload["department"] = obj.Value;
      }
      if (obj.Name === "custom:yoa") {
        payload["yoa"] = obj.Value;
      }
      if (obj.Name === "custom:course") {
        payload["course"] = obj.Value;
      }
      if (obj.Name === "custom:islateral") {
        payload["islateral"] = obj.Value;
      }
    });
    const params = {
      FunctionName: "arn:aws:lambda:ap-south-1:460459130467:function:CreateLC-dev",
      Payload: JSON.stringify(payload)
    };
    let lambda = new Lambda();
    await lambda.invoke(params).promise();
    const response = { message: "User Verified successfully" };
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
...
app.get("/listUsers", async (req, res, next) => {
  if (!req.query.filter) {
    const err = new Error("Type something to search");
    err.statusCode = 400;
    return next(err);
  }
  ...
  try {
    let response;
    if (req.query.token) {
      response = await listUsers(req.query.filter, req.query.token);
    } else {
      response = await listUsers((Filter = req.query.filter));
    }
    res.status(200).json(response);
  } catch (err) {
    next(err);
